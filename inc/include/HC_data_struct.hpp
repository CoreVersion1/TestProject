...
typedef struct _OutputPoint
{
    ...
    UINT64 u64DeviceSendTimeStampS;                 //模组发送的时间戳
...
//错误码
 enum ErrorCode{
    ...
    ERROR_OPTICAL_CMD_FAIL,                             //读取光学参数失败             
    ERROR_UNKNOW,       
};
...
{
    /* data */
    unsigned char data[128*160];
}stImgData;

//光学参数
typedef  struct _OpticalAngleParameter{
    float fPitch;      //俯仰角
    float fRoll;       //翻滚角
    float fYaw;        //偏航角
}stOpticalAngleParameter;
