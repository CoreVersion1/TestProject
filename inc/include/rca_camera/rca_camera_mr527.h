#ifndef __RCA_CAMERA_MR527_H__
#define __RCA_CAMERA_MR527_H__
...
#if defined(UART_API_HAVE_MR527_TOF_RGB)
...
#if defined(UART_API_HAVE_MR527_TOF_RGB) || defined(RR_DRV_HW_MR527_TOF)
...
#define CAMERA_TOF_TESTINFO_SECTOR          1//align by 4096 bytes sector
//#else
...
#if defined(RR_HW_ENABLE_ITOF_IRS2381)
#define BACK_TOF_WP_GPIO   136 //PE8 for r50
#else
...
#define UP_ITOF_INIT_ERROR     (-3)

#if defined(RR_DRV_HW_MR527_TOF)
typedef enum {
    TOF_SLAVE = 0,
    TOF_MASTER,
} TOF_ID;
...
typedef enum {
    RCA_CAMERA_TOF_TYPE_OMS = 0,
    RCA_CAMERA_TOF_TYPE_CHAOFENG,
    RCA_CAMERA_TOF_TYPE_UNKNOWN = 10,
} rca_camera_tof_type_id_e;

typedef enum {
    RCA_BOARDID_TYPE_OMS = 0,
    RCA_BOARDID_TYPE_CHAOFENG,
    RCA_BOARDID_TYPE_UNKNOWN = 10,
} rca_boardid_type_id_e;

typedef struct rca_camera_boardid_tof_type {
    rca_camera_tof_type_id_e camera_tof_type;
    rca_boardid_type_id_e boardid_type;
} BoardId_Tof_Type_st;
...
extern int rca_camera_get_boardid_and_tof_type(BoardId_Tof_Type_st *type);

#if defined(RR_HW_ENABLE_ITOF_IRS2381)
#define ITOF_INIT_ERROR    (-3)
#define ITOF_2381_WIDTH         (224)
#define ITOF_2381_HEIGHT        (173*9)
#define UP_ITOF_I2C        ("/dev/i2c-1")
#define UP_TIOF_WP_GPIO    310 //PJ22--(10-1)*32 + 22

#define ITOF_IRS2875C_WIDTH         (240)
#define ITOF_IRS2875C_HEIGHT        (181*9)
#define ITOF_IRS2875C_ID            (0x2875)
#define ITOF_IRS2381_ID             (0x2381)

extern int rca_itof_init(TOF_INIT *init);
extern void rca_itof_exit();
extern int rca_itof_start_capture(rca_tof_capture_callback cb);
extern int rca_itof_stop_capture();
extern int rca_itof_read_calib(unsigned char *buf, unsigned int len);
extern int rca_itof_write_calib(unsigned char *buf, unsigned int len);
extern Tof_Read_Flash_Type_e rca_itof_flash_read_calib_data_retry_file(
    tof_SLCResultBin_st *cali_data);
extern int rca_get_up_itof_id(void);
extern int rca_read_itof_sn(unsigned char *sn, int len);
extern int rca_read_itof_reg(unsigned char *msg, unsigned address);
extern ITOF_TYPE rca_read_itof_type(void);
#endif
#endif
#endif
