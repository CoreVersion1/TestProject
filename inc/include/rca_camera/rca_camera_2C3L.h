#ifndef __RCA_CAMERA_2C3L_H__
#define __RCA_CAMERA_2C3L_H__
...
typedef enum {
    ...
    RCA_CAMERA_FRAME_LINE_M,
    RCA_CAMERA_FRAME_CLOSE1,
    RCA_CAMERA_FRAME_LINE_R,
    RCA_CAMERA_FRAME_LINE_M1,
    RCA_CAMERA_FRAME_CLOSE2,
    RCA_CAMERA_FRAME_LINE_M2,
    RCA_CAMERA_FRAME_CLOSE3,
    RCA_CAMERA_FRAME_LINE_R1,
...
#pragma pack(1)
typedef struct {
    unsigned int version;               // calibration bin version
    PlanePara left;                         // left slight plane in cam coordinate
    PlanePara right;                        // right slight plane in cam coordinate
    PlanePara mid;                          // mid slight plane in cam coordinate
    PlanePara ground;                       // ground slight plane in cam coordinate
    SlightParams
    left_angs;             // left slight plane angles in cam coordinate
    SlightParams
    right_angs;            // right slight plane angles in cam coordinate
    SlightParams mid_angs;              // mid slight plane angles in cam coordinate
    CamExtrinsic cam;                   // camera extrinsic params
    float cam_lds_yaw;                  // cam-direction at left of lds-direction: positive
    float left_slight_grd_roll;         // positive: rotate-to-right(clockwise), negative: rotate-to-left. negative is
    // like this shape: /
    float right_slight_grd_roll;        // positive: rotate-to-right(clockwise), negative: rotate-to-left. negative is
    // like this shape: /
    float temperature_compensate_info;  // robot temperature when calibration
    float reserve[64];
    rrCameraHeadInfoSt cameraHead;
} CamSlightParams;
#pragma pack()
...
#define RCA_CAMERA_FRAME_END            RCA_CAMERA_FRAME_LINE_R1

#define camera_para_st camera_para_v3_st
