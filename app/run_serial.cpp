...
int main(int argc, char *argv[])
{
  ...
  {
    std::cerr << "Usage: " << argv[0] << " -d <serial_device> <-l|-r|-t> [-b <baud_rate>]"
              << std::endl;
    std::cerr << "Example: " << argv[0] << " -d /dev/ttyUSB0 -l -b 115200" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::cout << "Serial port " << serial_dev << " opened and configured with baud rate " << baud_rate
            << "." << std::endl;
